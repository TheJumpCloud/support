...
Function Connect-JCOnline ()
{
  <#
  .SYNOPSIS
    The Connect-JConline function sets the global variable $JCAPIKEY and does a GET API call to the /api/v2/directories endpoint to verify the API key.
  .DESCRIPTION
        ...
        Write-Host 'Do you want to bind your new users to existing JumpCloud systems during import?'
                ...
                while ($ConfirmSystem -ne 'Y' -and $ConfirmSystem -ne 'N')
                {
                    ...
                    if ($ConfirmSystem -eq 'Y')
                    {
                        ...
                        $ExistingSystems = Get-JCSystem | select HostName, DisplayName, @{Name='SystemID';Expression={$_._id}}, lastContact
...
function Import-JCUsersFromCSV ()
{
    ...
    begin
    {
        ...
        if ($JCAPIKEY.length -lt 40) {Connect-JConline}
...
"@
        ...
        $SystemCount = $NewUsers.SystemID | Where-Object Length -gt 1 | select -unique
        ...
        ForEach ($User in $NewUsers) {
            ...
            $Groups = $User | Get-Member -Name Group* | select Name
...
"@
    ...
    process
    {
        foreach ($UserAdd in $NewUsers)
        {
            ...
            $CustomAttributes = $UserAdd | Get-Member -Name *Attribute* | Where-Object {$_.Definition -NotLike "*=" -and $_.Definition -NotLike "*null"} | select Name
            ...
            if ($CustomAttributes.name.count -gt 1)
            {
                ...
                {   
                    ...
                    try #User is created
                    {
                        ...
                        $CustomGroups = $UserAdd | Get-Member -Name *Group* | select Name
    ...
    process

        {
...

            {
                ...
                $result = Get-JCcommandresult -ByID $CommandResultID | select -ExpandProperty Name #may need to modify this
    ...
    process

        {
...

            {
                ...
                try {
                    $GroupRemove = $GroupAdd = Invoke-RestMethod -Method POST -Body $jsonbody -Uri $GroupsURL -Header $hdrs
                    $Status = 'Removed'
                }
                catch {
                    $Status = $_.ErrorDetails
...
Function Get-JCSystemUser ()
{
    ...
    process
    {
        $URI = "https://console.jumpcloud.com/api/v2/systems/$SystemID/users"
            ...
            foreach($param in $PSBoundParameters.GetEnumerator())
            {
                ...
                if ($param.Key -like 'Attribute*')
                {
                    $CustomAttribute =  [pscustomobject]@{
                        ...
                        $UniqueAttributes = $CustomAttributeArrayList | select CustomAttribute -Unique

                        $NewAttributes = New-Object System.Collections.ArrayList

                            foreach ($A in $UniqueAttributes )
                            {
                                $Props = $CustomAttributeArrayList | ? CustomAttribute -EQ $A.CustomAttribute
    ...
    process
        {
            ...
            elseif ($PSCmdlet.ParameterSetName -eq 'Username' -and ($NumberOfCustomAttributes))
            {
...

                {
                    ...
                    $CurrentAttributes = Get-JCUser -UserID $URL_ID | select -ExpandProperty attributes | select value, name
            ...
            elseif ($PSCmdlet.ParameterSetName -eq 'ByID' -and ($NumberOfCustomAttributes))
            {
                ...
                $CurrentAttributes = Get-JCUser -UserID $UserID | select -ExpandProperty attributes | select value, name
                Write-Debug "There are $($CurrentAttributes.count) existing attributes"
