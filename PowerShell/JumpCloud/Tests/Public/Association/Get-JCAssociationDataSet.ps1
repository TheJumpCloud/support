...
#         $EmptySources = @()
#         ForEach ($JCAssociationType In $JCAssociationTypes) # | Where-Object { $_.TypeName.TypeNameSingular -eq 'system_group' })
#         {
...
#         ####################################################################################################
#         # # Export data to file
#         # ($AssociationDataSet | ConvertTo-Json -Depth:(100)) | Out-File -Path:($PSScriptRoot + '/Get-JCAssociation.Tests.PeBigOrgster.json') -Force
