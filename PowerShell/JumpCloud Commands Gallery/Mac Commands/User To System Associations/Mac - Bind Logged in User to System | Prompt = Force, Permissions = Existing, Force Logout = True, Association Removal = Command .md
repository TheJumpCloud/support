...
Mac - Bind Logged in User to System | Prompt = Force, Permissions = Existing, Force Logout = True, Association Removal = Command | JCCG v1.0
...
After importing this command the variables `JCAPIKey=''` and `commandID=''` must populated before the command can be run.
...
```
Import-JCCommand -URL 'https://git.io/fh2jT'
```
