...
$PesterResults =Invoke-Pester -Path ($PSScriptRoot + '/Tests/') -PassThru -Tag:($IncludeTags) -ExcludeTag:($ExcludeTagList) | ConvertTo-NUnitReport -AsString | Out-File -Path "' + $moduleName + '-TestResults.xml)"') | Set-Content -Path:($testModulePath)
