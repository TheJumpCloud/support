...
$PesterResults = Invoke-Pester -Path ($PSScriptRoot) -Parameters  -PassThru -Tag:($IncludeTags) -ExcludeTag:($ExcludeTagList)
$PesterResults | ConvertTo-NUnitReport -AsString | Out-File -FilePath:("$($ModuleName)-$($env:Agent.OS)-TestResults.xml") -Force
