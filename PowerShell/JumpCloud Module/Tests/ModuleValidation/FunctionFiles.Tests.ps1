Describe -Tag:('ModuleValidation') 'Function Format Tests' {
    ...
    $FunctionList = Get-FunctionReport -Folder:(($ModuleRoot + '/Public'), ($ModuleRoot + '/Private')) | Where-Object { $_.FileName -notlike 'ScriptBlock_*' -and -not ($PSVersionTable.PSEdition -eq 'Desktop' -and $_.FileBaseName -eq 'Get-JCEvent' ) }
