Describe -Tag:('MSP') 'Get-JCPolicyGroupTemplate' {
    ...
    Context "Should Remove Tests" {
        ...
        It "Removes a policy group template by ID" {
            ...
            # Get the group template after it's been removed this should throw not found
            { Get-JCPolicyGroupTemplate -GroupTemplateID $newPolicyGroupTemplate.id } | Should -Throw
        }
    }
    Context "Should not remove if the object doesn't exist tests" {
