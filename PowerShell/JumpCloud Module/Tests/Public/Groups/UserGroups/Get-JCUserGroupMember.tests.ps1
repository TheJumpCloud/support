...
Describe -Tag:('JCUserGroupMember') 'Get-JCUserGroupMember Parallel' {
    BeforeAll {
        Connect-JCOnline -JumpCloudApiKey:($PesterParams_ApiKey) -force | Out-Null
        If (-not (Get-JCUserGroupMember -GroupName:($PesterParams_UserGroup.Name)))
        {
    ...
    It 'Gets a User Groups membership by Groupname' {
        $UserGroupMembers = Get-JCUserGroupMember -GroupName:($PesterParams_UserGroup.Name) -parallel $true
        $UserGroupMembers.UserID.Count | Should -BeGreaterThan 0
    }
    It 'Gets a User Groups membership -ByID' {
        $UserGroupMembers = Get-JCUserGroupMember -ByID $PesterParams_UserGroup.Id -parallel $true
        $UserGroupMembers.UserID.Count | Should -BeGreaterThan 0
    }
}
