...
Describe -Tag:('JCSystem') "Get-JCSystem 1.15.2" {
    It "Searches for a JumpCloud system using filterDateProperty lastContact and after" {
        ...
        $NewestSystemlastContactVerify = ([DateTime]::Parse((Get-JCSystem -filterDateProperty lastContact -dateFilter after -date $NewestSystemlastContact.addSeconds(-1) | Select-Object -Last 1 -ExpandProperty lastContact))).ToUniversalTime()
    ...
    It "Searches for a JumpCloud system using filterDateProperty lastContact and before" {
        ...
        $OldestSystemlastContactVerify = ([DateTime]::Parse((Get-JCSystem -filterDateProperty lastContact -dateFilter before -date $OldestSystemlastContact.addSeconds(1) | Select-Object -First 1 -ExpandProperty lastContact))).ToUniversalTime()
