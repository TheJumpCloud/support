...
Describe -Tag:('JCSystem') "Get-JCSystem 1.15.2" {
    It "Searches for a JumpCloud system using filterDateProperty lastContact and after" {
        $NewestSystemlastContact = ([DateTime]::Parse((Get-JCSystem -returnProperties lastContact | Where-Object lastContact -ne $null | Select-Object -Last 1 -ExpandProperty lastContact))).ToUniversalTime()
