Describe -Tag:('JCCloudDirectory') 'Remove-JCOffice365Member' {
    ...
    It 'Attempts to remove a non-existent user' {
        ...
        { $User = Remove-JCOffice365Member -Name $Directories.Name -UserID 123456 } | Should -Throw
        # $User.Status | Should -BeLike 'Bad Request*' #TODO: status is not populated
    }
    It 'Attempts to remove a non-existent group' {
        { $Group = Remove-JCOffice365Member -Name $Directories.Name -GroupName 'Dummy Group' } | Should -Throw
        { $Group = Remove-JCOffice365Member -Name $Directories.Name -GroupID 123456 } | Should -Throw
