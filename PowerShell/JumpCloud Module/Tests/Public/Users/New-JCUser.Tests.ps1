Describe -Tag:('JCUser') 'New-JCUser 1.0' {
    ...
    It "Creates a new user" {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser
        $NewUser._id.count | Should -Be 1
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User allow_public_key -eq True " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -allow_public_key $true
        $NewUser.allow_public_key | Should -Be True
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User allow_public_key -eq False " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -allow_public_key $false
        $NewUser.allow_public_key | Should -Be False
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User sudo -eq True " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -sudo $true
        $NewUser.sudo | Should -Be True
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User sudo -eq False " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -sudo $false
        $NewUser.sudo | Should -Be False
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User enable_managed_uid -eq True " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -enable_managed_uid $true -unix_uid 1 -unix_guid 1
        $NewUser.enable_managed_uid | Should -Be True
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User enable_managed_uid -eq False " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -enable_managed_uid $false
        $NewUser.enable_managed_uid | Should -Be False
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User passwordless_sudo -eq True " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -passwordless_sudo $true
        $NewUser.passwordless_sudo | Should -Be True
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User passwordless_sudo -eq False " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -passwordless_sudo $false
    ...
    It "Creates a new User ldap_binding_user -eq True " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -ldap_binding_user $true
        $NewUser.ldap_binding_user | Should -Be True
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User ldap_binding_user -eq False " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -ldap_binding_user $false
        $NewUser.ldap_binding_user | Should -Be False
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User enable_user_portal_multifactor -eq True " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -enable_user_portal_multifactor $true
        $NewUser.enable_user_portal_multifactor | Should -Be True
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User enable_user_portal_multifactor -eq False " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -enable_user_portal_multifactor $false
        $NewUser.enable_user_portal_multifactor | Should -Be False
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User sets unix_uid" {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -unix_uid 100
        $NewUser.unix_uid | Should -Be 100
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User sets unix_guid" {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser  -unix_guid 100
        $NewUser.unix_guid | Should -Be 100
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }
    It "Creates a new User sets manager" {
        $managerID = $PesterParams_User1.id
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -manager $managerID
    ...
    It "Creates a new User sets managerUsername" {
        ...
        $managerID = $PesterParams_User1.id
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -manager $managerUsername
        $NewUser.manager | Should -Be $managerID
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }
    It "Creates a new User sets managerEmail" {
        ...
        $managerID = $PesterParams_User1.id
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -manager $managerEmail
        $NewUser.manager | Should -Be $managerID
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }
    It "Creates a new User sets alternateEmail" {
        $alternateEmail = "$((New-RandomString -NumberOfChars 6))ae@DeleteMe.com"
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -alternateEmail $alternateEmail
    ...
    It "Creates a new User sets recoveryEmail" {
        ...
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -recoveryEmail $recoveryEmail
    ...
    It "Creates a new User sets managedAppleID" {
        ...
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -managedAppleID $managedAppleID
        $NewUser.managedAppleID | Should -Be $managedAppleID
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User with 1 custom attributes" {
        $NewUser = New-RandomUser -Attributes -Domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -NumberOfCustomAttributes 1
        $NewUser.attributes._id.Count | Should -Be 1
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User with 3 custom attributes" {
        $NewUser = New-RandomUser -Attributes -Domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -NumberOfCustomAttributes 3
        $NewUser.attributes._id.Count | Should -Be 3
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }

    It "Creates a new User with state SUSPENDED" {
        $NewUser = New-RandomUser -Attributes -Domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -state "SUSPENDED"
    ...
    It "Creates a new User with state STAGED" {
        $NewUser = New-RandomUser -Attributes -Domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -state "STAGED"
...
Describe -Tag:('JCUser') 'Add-JCUser 1.3.0' {
    ...
    It "Adds a JumpCloud user with a high UID and GUID" {
        ...
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -unix_uid 1000000 -unix_guid 1000000
    ...
    It "Adds a JumpCloud user with password_never_expires false " {
        ...
        $ExpFalse = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -password_never_expires $false
    ...
    It "Adds a JumpCloud user with password_never_expires true " {
        ...
        $ExpTrue = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -password_never_expires $true
...
Describe -Tag:('JCUser') "New-JCUser MFA with enrollment periods 1.10" {
    ...
    It "Creates a new user with enable_user_portal_multifactor -eq True and a 30 days specified for EnrollmentDays" {
        ...
        $Newuser = New-RandomUser -Domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -enable_user_portal_multifactor $true -EnrollmentDays $EnrollmentDays
    ...
    It "Creates a new user with enable_user_portal_multifactor -eq True with Attributes" {
        ...
        $NewUser = New-RandomUser -Domain "delNewUser.$(New-RandomString -NumberOfChars 5)"-Attributes | New-JCUser -NumberOfCustomAttributes 2 -enable_user_portal_multifactor $True
    ...
    It "Creates a new user with enable_user_portal_multifactor -eq True and a 30 days specified for EnrollmentDays with Attributes" {
        ...
        $NewUser = New-RandomUser -Domain "delNewUser.$(New-RandomString -NumberOfChars 5)"-Attributes | New-JCUser -NumberOfCustomAttributes 2 -enable_user_portal_multifactor $True -EnrollmentDays $EnrollmentDays
...
Describe -Tag:('JCUser') "New-JCUser with suspend param 1.15" {
    ...
    It "Creates a new User suspended -eq True " {
        $NewUser = New-RandomUser -domain "delNewUser.$(New-RandomString -NumberOfChars 5)" | New-JCUser -suspended $true
        $NewUser.suspended | Should -Be True
        Remove-JCUser -UserID $NewUser._id -ByID -Force
    }
