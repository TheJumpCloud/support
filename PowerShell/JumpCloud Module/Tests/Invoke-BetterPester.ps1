Param(
    ...
    , [Parameter(Mandatory = $false, ValueFromPipelineByPropertyName = $true, Position = 3)][ValidateSet("Get", "Set", "New", "Other")][System.String[]]$ParallelTagType
...
ForEach ($PesterTest In $PesterTests)
{
    ...
    ForEach ($DescribeLine In $DescribeLines)
    {
        If ($DescribeLine.Line -match 'Tag')
        {
            ...
            if ($PesterTestFullName -Match "Get"){
                # Write-Host "Get Test"
                $GetTags += "$TagParameterValue"
            }
            elseif ($PesterTestFullName -Match "Set"){
                # Write-Host "Put Test"
                $SetTags += "$TagParameterValue"
            }
            elseif ($PesterTestFullName -Match "New"){
                # Write-Host "Post Test"
                $NewTags += "$TagParameterValue"
            }
            else{
                # Write-Host "OtherTest"
                $OtherTags += "$TagParameterValue"
            }
        }
        Else
        {
...
Write-Host "Get Tags Count:"$GetTags.Count
Write-Host "Set Tags Count:"$SetTags.Count
Write-Host "New Tags Count:"$NewTags.Count
Write-Host "Other Tags Count:"$OtherTags.Count
# Filters on tags
