...
if [ "$oldUser" = "$newUser" ]; then
    echo "Current username = $oldUser -eq new username $newUser"
    echo "No updates needed"
    exit 0
 fi

#Query existing user accounts
existingUsers=($(dscl . -list /Users | grep -Ev "^_|com.*|root|nobody|daemon|\/" | cut -d, -f1 | sed 's|CN=||g'))

#Ensure old user account is correct and account exists on system 
if [[ ! " ${existingUsers[@]} " =~ " ${oldUser} " ]] 
then
...
if [[ " ${existingHomeFolders[@]} " =~ " ${newUser} " ]] 
then
...
#Restarts the Menu Bar app to show the updated username in the fast user switcher drop down
